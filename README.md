...
-   **フレームワーク：** LangChain 0.2.17  
    -   LangChainは現在のLLMアプリケーション開発における事実上の標準フレームワークであり、LangSmithなどの評価ツールとの連携が容易。  
        ...
        LangSmithによる評価との統合がLangChainほど成熟していないため、本構成では採用見送り。

-   **クラウド：** AWS  
    -   自身のAWS利用経験があり、慣れた環境で迅速に開発を進められるという前提に加え、Amazon Bedrockを通じてClaudeモデルを利用できる点を重視。  
        Claudeモデルは日本語の文脈理解と忠実かつ簡潔な回答生成に優れており、本プロジェクトの要件である「社内FAQ」に最適であると判断。
        
-   **埋め込みモデル：** Titan Text Embeddings v2    
    -   RAGに最適化された設計で、次元サイズを選択して精度、コスト、レイテンシのバランスが取れるため採用。    
        他モデルのCohereは英語、多言語に特化しているため候補から除外。
        
-   **ベクトルDB：** Amazon OpenSearch Serverless      
    -   当初は複数のサービスと連携出来るAmazon Kendraを利用していましたが、テスト中に高額のAPI料がかかるため断念。    
        Amazon Aurora（SQL連携）やNeptune（グラフ分析）は用途面で本プロジェクトの要件と合致していないため、  
        コスト効率に優れたAmazon OpenSearch Serverlessを採用。 
　　  
-   **言語モデル：** Claude 3.5 Sonnet 20241022 v2        
    -   AWS東京リージョンで利用可能なモデルから選定。言語モデルは最新版でもバグが起こりにくく、このモデルはトークンコストも据え置かれるため、性能とコストのバランスが良い最新のClaude 3.5 Sonnetを採用。  
        低コストな 3.0 Haikuも検討したが、求める処理能力を考慮し、総合的なパフォーマンスを重視。    

-   **LLM評価：** Ragas 0.2.0, LangSmith 0.1.112  
    -   RAGシステムの網羅的な評価体制を構築するため、管理基盤のLangSmithと評価フレームワークのRagasを連携させて採用。  
        LangSmithで実行ログやプロンプトを管理・可視化し、Ragasを用いて回答の忠実性などを定量的に評価することで、課題の特定と改善を重視。 
## ディレクトリ構成
```
...
1.  **(ドキュメントの情報の格納)**
    -   事前にドキュメントをチャンク分割し、ベクトル化してベクトルDBにインデックス化して格納。  
        一連の作業はBedrock Knowledge Baseで行う。
2.  **(質問)**
    -   ユーザーの質問を送信し、アプリケーション側で受信。
3.  **(ベクトル検索)**
...
質問文： "給与の支給日はいつですか？"  
想定回答： "給与の支給日は原則として毎月25日です。ただし、支給日が金融機関休業日の場合は、前営業日となります。"
...
## 苦労した点
-   API利用料のコスト管理  
    -   最初のベクトルDBにはAmazon Kendraを採用してテストしていたが、料金表を確認すると2万円以上超過。  
        Amazon Kendraは時間あたりのコストがAmazon OpenSearch Serverlessと比較して約5倍程度高いため、個人開発でテストを継続するには費用対効果が低いと判断して切り替えて解決。  
        この経験から、技術選定では性能だけでなく、利用実態に合った課金体系を理解することの重要性を痛感。

-   RAGアーキテクチャーの理解  
    -   ポートフォリオ作成当初のRAG設計では、期待する回答結果との間に大きな乖離が生じた。(回答テストエラー、回答品質など)  
        この課題に対し、Lambda関数によるテスト環境を活用、RagasやLangSmithでの評価を繰り返すことで設計を柔軟に見直し、品質向上を実現。  
        AI開発においては初期設計に固執せず、実践的なテストと継続的な評価・改善サイクルを回すことが不可欠だと認識。

-   AWSのレート制限対策  
    -   RagasやLangSmithを用いた評価スクリプト実行中に、APIのレート制限エラーが発生。  
        対策としてリクエスト間に適切な待機時間の処理を挿入し、さらにエラー発生時の自動リトライ機能を実装することで「エラー率:0%」を達成。
